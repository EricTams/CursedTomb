...
**Player physics (`src/player.ts`)** is a hand-rolled AABB-vs-grid platformer: axis-separated `moveX`/`moveY` sweeps, coyote time, jump buffering, one-way platforms (drop-through on down+jump), ladders, and the **swing** system (latch a grapple ring, then free-fall until the rope reaches its default swing length and snaps taut into a rigid pendulum; either button exits, releasing with tangential momentum). The whip is a line segment (`whipSegment`) tested against enemy AABBs (`segmentHitsRect`, Liang-Barsky) and grapple rings (`tryLatch`).
